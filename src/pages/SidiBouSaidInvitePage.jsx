...
const makeClickHandler = (sectionId, elementId) => (e) => {
  if (!elementId) return;
  e.stopPropagation();
  const cleanSectionId = sectionId && sectionId.startsWith('sec-') ? sectionId.replace('sec-', '') : sectionId;
  console.log("makeClickHandler triggered", cleanSectionId, elementId);
  if (window.parent && window.parent !== window) {
    window.parent.postMessage({
      type: 'ELEMENT_CLICKED',
      payload: { sectionId: cleanSectionId, elementId }
    }, "*");
  }
};
...
const Section = ({ invite, startY, height, bg = "transparent", layerNames = [], delayOffset = 200, isFullScreenHeight = false, children, ...rest }) => {
  ...
  const textLayer = ({ id, left, top, width: w, children: textChildren, color = blue, fontSize = 16, lineHeight, family = "Cormorant Infant, serif", weight = 400, style = "normal", align = "center", letterSpacing = 0, wordSpacing = "normal", transform, zIndex = 3, pointerEvents, reveal = false, delay = 0 }) => {
    ...
    return (
    <div
      onClick={makeClickHandler(rest['data-section-id'], id)}
      ...
      style={{
        ...
        whiteSpace: "pre-wrap",
        zIndex,
        pointerEvents: pointerEvents || (id ? "auto" : "none"),
